-- | sensenet//forge Product Page
-- | Code hosting + review for the agent era
-- | Full product marketing page (armory shape)
module Straylight.Pages.Products.SensenetForge where
...
sensenetForgePage :: forall q i o m. H.Component q i o m
sensenetForgePage = H.mkComponent
    ...
    [ HH.div
          ...
          HH.div
            ...
            [ HH.div
                ...
                [ HH.code
                    ...
                    , HH.text "curl -fsSL forge.straylight.software | sh"
                    ]
                , HH.button
            ...
            , HH.div
                ...
                [ HH.code
                    [ cls [ "text-[#dde6f0] whitespace-nowrap" ] ]
                    [ HH.span [ cls [ "text-[#596775]" ] ] [ HH.text "$ " ]
                    , HH.text "nix run github:straylight-software/forge"
                    ]
                , HH.button
                    [ cls [ "text-[#596775] hover:text-[#c4b5fd] transition-colors text-xs shrink-0" ] ]
                    [ HH.text "copy" ]
        ...
        , -- Social proof
          HH.p
            ...
            [ HH.span [ cls [ "text-violet-400/60" ] ] [ HH.text "replaces " ]
            , HH.text "GitHub, Gerrit, Phabricator"
            ]
        ]
    ]
            ...
            [ HH.table
                ...
                [ HH.thead_
                    [ HH.tr
                        ...
                        , HH.th [ cls [ "py-4 text-center text-muted-foreground font-medium" ] ] [ HH.text "Phabricator" ]
                        , HH.th [ cls [ "py-4 text-center text-muted-foreground font-medium" ] ] [ HH.text "GitLab" ]
                        ]
                    ]
                , HH.tbody_
                    [ comparisonRow "Review model" "Stacked diffs" "PRs" "Patchsets" "Diffs" "MRs"
                    , comparisonRow "jujutsu support" "Native" "no" "no" "no" "no"
                    , comparisonRow "Agent provenance" "Attestation" "no" "no" "no" "no"
                    , comparisonRow "Semantic search" "Built-in" "no" "no" "no" "no"
                    , comparisonRow "Stack rebasing" "Automatic" "manual" "manual" "manual" "manual"
                    , comparisonRow "Branch requirement" "None" "required" "optional" "required" "required"
                    , comparisonRow "Conflict resolution" "jj native" "git merge" "git merge" "arc" "git merge"
                    , comparisonRow "Self-hosted" "Yes" "Enterprise" "Yes" "Yes" "Yes"
                    ]
                ]
            ]
        , HH.p
            [ cls [ "text-center text-muted-foreground text-xs mt-6" ] ]
            [ HH.text "Feature comparison as of 2024. Phabricator is EOL but still widely deployed." ]
        ]
    ]

comparisonRow :: forall w i. String -> String -> String -> String -> String -> String -> HH.HTML w i
comparisonRow feature us github gerrit phab gitlab =
  HH.tr
    [ cls [ "border-b border-border" ] ]
    ...
    , HH.td [ cls [ "py-3 text-center" ] ] [ compCell phab ]
  ...
  HH.section
    ...
    [ HH.div
        ...
        , codeBlock
            ...
            , codeLine "$ " "nix profile install github:straylight-software/forge"
            , HH.text "\n"
            , codeLine "# " "Or via curl"
            , codeLine "$ " "curl -fsSL https://forge.straylight.software/install.sh | sh"
            , HH.text "\n"
            ...
            , HH.text "\n"
            , codeLine "# " "Stack another change on top"
            , codeLine "$ " "jj new -m 'Add feature Y (depends on X)'"
            , codeLine "$ " "forge diff create"
            , HH.text "\n"
            , codeLine "# " "Submit the stack for review"
  ...
  HH.section
    ...
    [ HH.div
        ...
        , HH.p
            ...
            [ HH.text "sensenet//forge is in private beta. Join the waitlist for early access." ]
        , HH.div
            [ cls [ "flex flex-col sm:flex-row items-center justify-center gap-4" ] ]
            [ primaryButton "/waitlist" "Join the waitlist"
